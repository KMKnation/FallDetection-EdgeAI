...
class MobiDataGenerator(keras.callbacks.Callback):
    ...
    def feed_csv(self, columns, file, ADL, SUBJECT_ID, TRIAL_NO):
        ...
        col_indexes = [df.columns.get_loc(column) for column in self.cols_to_train]
        col_indexes.sort()
        # self._all_data = self._all_data.append(df.iloc[0:800], ignore_index=True, sort=True)
        min_limit = 800
        if len(df.iloc[0:min_limit, col_indexes].values) >= min_limit:
            self.features.append(df.iloc[0:min_limit, col_indexes].values)
            self.labels.append(self.label_to_numeric(ADL))
